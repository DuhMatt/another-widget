...
object Preferences : KotprefModel() {
    ...
    var weatherTempUnit by stringPref(key = "PREF_WEATHER_TEMP_UNIT", default = if (ConfigurationCompat.getLocales(context.resources.configuration)[0]?.isMetric() != false) "C" else "F")
    var weatherRealTempUnit by stringPref(key = "PREF_WEATHER_REAL_TEMP_UNIT", default = if (ConfigurationCompat.getLocales(context.resources.configuration)[0]?.isMetric() != false) "C" else "F")
    ...
    var weatherProvider by intPref(default = if (ConfigurationCompat.getLocales(context.resources.configuration)[0]?.isMetric() != false) Constants.WeatherProvider.YR.rawValue else Constants.WeatherProvider.WEATHER_GOV.rawValue)
